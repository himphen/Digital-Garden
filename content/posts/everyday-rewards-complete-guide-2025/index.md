...
如果你是重度 Woolworths 用戶，可以考慮升級到 Everyday Extra。這是一個付費訂閱服務，每月費用 AUD 7，包括以下具體福利：
...
Everyday Rewards 提供了一個很貼心的功能：你可以選擇不立即兌換積分，而是儲存起來在特定時間使用。這個功能特別適合聖誕節購物季，當你需要大量購物時，可以一次使用所有累積的積分。

要使用這個功能，你需要在 App 中設定積分儲存偏好，選擇「Save for Christmas」選項。這樣系統就不會在你達到 2000 積分時提供 AUD 10 折扣，而是讓你繼續累積積分。
...
{{< figure
    class="img-sm"
    src="screenshot-2.png"
    alt="Google Pay 直接支援 NFC 拍卡，可以不用掃描會員卡的 Barcode。"
    caption="Google Pay 直接支援 NFC 拍卡，可以不用掃描會員卡的 Barcode。"
    >}}

我建議養成每次購物都使用會員卡的習慣，即使是很小的購買也不要忽略。積分的累積是一個長期過程，每一次的小額積分都有價值。

定期檢查 App 中的 Boost 優惠，並據此調整購物計劃。我通常在週一晚上查看週三開始的優惠，然後制定購物清單。這種前瞻性的規劃讓我能夠最大化積分收益。

將 Everyday Rewards 卡添加到手機 Wallet 裡，比如 Apple Wallet 或 Google Pay。這樣即使忘記帶實體卡或手機沒有網路，也能使用會員卡。
